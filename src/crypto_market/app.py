...
from . import backtest, crossex, data, funding, hourly, intraday, onchain
...
def cross_exchange(
    ...
    live = crossex.episodes(crossex.ticks(hours), fee, latency_ms)
    collector = getattr(app.state, "collector", None)
    live["collector"] = {"since": collector.started, "counts": collector.counts} if collector else None
    live["pairs"] = [{k: (_clean(v) if not isinstance(v, list) else v) for k, v in p.items()} for p in live["pairs"]]
    return {"minutes": {**{k: _clean(v) for k, v in g.items()}, "series": series, "fee_sweep": sweep, "pairs": pairs},
            "live": live}
